...
import com.android.approjects.soundrecorder.filelist.FileListActivity;
import com.android.approjects.soundrecorder.util.FileUtils;
import com.android.approjects.soundrecorder.util.MediaRecorderWrapper;
import com.android.approjects.soundrecorder.util.PermissionUtils;
import com.android.approjects.soundrecorder.util.StorageUtils;
import com.android.approjects.soundrecorder.util.SystemPropertiesWrapper;
import com.android.approjects.soundrecorder.util.Utils;
...
class RemainingTimeCalculator {
    ...
    public long timeRemaining() {
        // Calculate how lon we can record based on free disk space.
...
public class SoundRecorder extends Activity
    ...
    private BroadcastReceiver mMountReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (mPath == StorageUtils.STORAGE_PATH_PHONE_INDEX
                ...
                mErrorUiMessage = "Storage is full";
                updateUi();
            }
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mWakeLock = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "SoundRecorder:wakelock");
        ...
        for (int j = 0; j < mPhoneCount; j++) {
            ...
            mPhoneStateListener[j] = getPhoneStateListener(j);
        }
    ...
    private class StartRecordingTask extends AsyncTask<RecordingParams, Integer, String> {
        ...
        protected void onPreExecute() {
            ...
            String message = "Starting recording";
        ...
        @Override
        protected String doInBackground(RecordingParams... recordingParams) {
            RecordingParams param = recordingParams[0];
            mRecorder.startRecording(param.mOutputFileFormat, param.mExtName,
                    param.mContext, param.mAudioSourceType, param.mCodecType);
        ...
        switch (viewId) {
            case R.id.recordButton:
                ...
                if (mPath == StorageUtils.STORAGE_PATH_PHONE_INDEX &&
                        !StorageUtils.isPhoneStorageMounted()) {
                    mSampleInterrupted = true;
                    mErrorUiMessage = "Phone storage unmounted";
                    updateUi();
                } else if (mPath == StorageUtils.STORAGE_PATH_SD_INDEX &&
                        !StorageUtils.isSdMounted(SoundRecorder.this)) {
                    mSampleInterrupted = true;
                    mErrorUiMessage = "Insert an SD card";
                    updateUi();
                } else {
                    if ((mCallState == TelephonyManager.CALL_STATE_OFFHOOK) &&
                            (mAudioSourceType == MediaRecorderWrapper.AudioSource.MIC)) {
                    ...
                    if (AUDIO_AMR.equals(mRequestedType)) {
                        ...
                        new StartRecordingTask().execute(new RecordingParams(
                                MediaRecorderWrapper.OutputFormat.THREE_GPP, ".3gpp",
                                this, mAudioSourceType, MediaRecorderWrapper.AudioEncoder.AMR_NB));
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if(event.getKeyCode() == KeyEvent.KEYCODE_6 && event.getAction() == event.ACTION_UP){
            ...
            if((mAudioSourceType == MediaRecorderWrapper.AudioSource.VOICE_CALL) ||
                ...
                String message = null;
                message = "AAC voice call recording is not supported";
        ...
        if((event.getKeyCode() == KeyEvent.KEYCODE_1 || event.getKeyCode() == KeyEvent.KEYCODE_2)
            ...
            if((mAudioManager.getMode() != AudioManager.MODE_IN_CALL) ||
                ...
                if(mAudioManager.getMode() != AudioManager.MODE_IN_CALL) {
                    message = "This option is supported, only while in call";
                } else {
                    message = "AAC voice call recording is not supported";
                }
    ...
    private void showSavedToast() {
        String info = "Recording has been stopped and saved";
        ...
        switch (mRecorder.state()) {
            case Recorder.IDLE_STATE:
                if (mRecorder.sampleLength() == 0) {
                    ...
                    if (true == bSSRSupported) {
                        mStateMessage2.setText("Press: 0-MIC 1-Rx 2-Tx+Rx 3-AMR 4-EVRC 5-QCELP\n" +
                                "    6-AAC_MP4 7-AAC_SSR 8-LPCM_SSR 9-AMR-WB A-AMR-WB-3GPP and Press record");
                    } else {
                        if (SystemPropertiesWrapper.getBoolean(VENDOR_SOUNDRECORDER_DEBUG_ENABLE, false)) {
                            // mStateMessage2.setText(res.getString(R.string.press_record));
                        } else {
                            mStateMessage2.setText("Press Record");
                        }
                    }
                    mVUMeter.resetAngle();
                    ...
                    mStateMessage1.setVisibility(View.INVISIBLE);
                    mStateMessage2.setVisibility(View.VISIBLE);
                    mStateMessage2.setText("Recording stopped");
            ...
            case Recorder.RECORDING_STATE:
                mRecordButton.setImageResource(R.drawable.ic_pause_black_36dp);
                mRecordButton.setEnabled(true);
                mRecordButton.setFocusable(true);
    ...
    public void onError(int error) {
        ...
        if (message != null) {
            new AlertDialog.Builder(this)
                    .setTitle(R.string.app_name)
                    .setMessage(message)
                    .setPositiveButton(android.R.string.ok, (true==isExit)?
                            (new DialogInterface.OnClickListener() {
