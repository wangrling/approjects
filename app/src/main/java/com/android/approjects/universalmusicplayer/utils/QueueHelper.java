...
import static com.android.approjects.universalmusicplayer.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_GENRE;
import static com.android.approjects.universalmusicplayer.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_SEARCH;
...
public class QueueHelper {
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueue(String mediaId,
        ...
        if (hierarchy.length != 2) {
            LogHelper.e("Could not build a playing queue for this mediaId: " + mediaId);
            return null;
        }
        String categoryType = hierarchy[0];
        String categoryValue = hierarchy[1];
        LogHelper.d("Creating playing queue for " + categoryType + ", " + categoryValue);
        ...
        if (tracks == null) {
            LogHelper.e("Unrecognized category type: " + categoryType + " for media " + mediaId);
            return null;
        }
    ...
    public static List<MediaSessionCompat.QueueItem>
            getPlayingQueueFromSearch(String query, Bundle queryParams, MusicProvider musicProvider) {

        LogHelper.d("Creating playing queue for musics from search: " + query + " params = " + queryParams);
    ...
    private static List<MediaSessionCompat.QueueItem> convertToQueue(
        ...
        for (MediaMetadataCompat track : tracks) {
            // We create a hierarchy-aware mediaID, so we know what hte queue is about by looking
    ...
    public static List<MediaSessionCompat.QueueItem> getRandomQueue(MusicProvider musicProvider) {
        ...
        LogHelper.d("QueueManager getRandomQueue: result.size = " + result.size());
