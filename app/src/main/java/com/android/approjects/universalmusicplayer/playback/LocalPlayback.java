...
/**
 * A class that implements local media playback using {@link ExoPlayer}
 */
// 使用ExoPlayer而不是MediaPlayer进行播放。
public class LocalPlayback implements Playback {
    ...
    public LocalPlayback(Context context, MusicProvider musicProvider) {
        ...
        // Create teh Wifi lock (this does not acquire the lock, this just creates it)
        this.mWifiLock = ((WifiManager) applicationContext.getSystemService(Context.WIFI_SERVICE))
                .createWifiLock(WifiManager.WIFI_MODE_FULL, "ump_lock");
    }
        ...
        if (mediaHasChanged || mExoPlayer == null) {
            ...
            DataSource.Factory dataSourceFactory =
                    new DefaultDataSourceFactory(
                            mContext, Util.getUserAgent(mContext, "ump"), null);
    ...
    public void seekTo(long position) {
        LogHelper.d("LocalPlayback seekTo called with " + position);
        if (mExoPlayer != null) {
            registerAudioNoisyReceiver();
    ...
    private void tryToGetAudioFocus() {
        LogHelper.d("tryToGetAudioFocus");
    ...
    private void configurePlayerState() {
        LogHelper.d("configurePlayerState. mCurrentAudioFocusState = " + mCurrentAudioFocusState);
    ...
    private final AudioManager.OnAudioFocusChangeListener mOnAudioFocusChangeListener =
            new AudioManager.OnAudioFocusChangeListener() {
                ...
                public void onAudioFocusChange(int focusChange) {
                    LogHelper.d("onAudioFocusChange. FocusChange = " + focusChange);
    ...
    private void releaseResources(boolean releasePlayer) {
        LogHelper.d("releaseResources. releasePlayer = " + releasePlayer);

        // Stops and release player (if requested and available).
    ...
    private final class ExoPlayerEventListener implements Player.EventListener {
        ...
        public void onPlayerError(ExoPlaybackException error) {
            ...
            LogHelper.e("ExoPlayer error: what = " + what);
            if (mCallback != null) {
